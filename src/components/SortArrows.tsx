import React from "react";

interface IShortArrows {
  name: string;
  sortObject: Object;
  onClick: Function;
}

/**
 * @isAscending = true => will show arrow-up
 * @isAscending = false => will show arrow-down
 * @isAscending = null => will disable arrow active
 * UI:
 * arrow-up active: is ascending
 * arrow-down active: is descending
 * no arrows active: normal
 */
const SortArrows = ({ sortObject, name, onClick }: IShortArrows) => {
  // @ts-ignore
  const isAscending = sortObject[name];
  return (
    <>
      <span
        className="ant-table-column-sorter ant-table-column-sorter-full"
        style={{ cursor: "pointer", userSelect: "none" }}
        onClick={() => onClick(name)}
      >
        <span className="ant-table-column-sorter-inner">
          <span
            role="img"
            aria-label="caret-up"
            className={`anticon anticon-caret-up ant-table-column-sorter-up ${
              isAscending === true && "active"
            }`}
          >
            <svg
              viewBox="0 0 1024 1024"
              focusable="false"
              data-icon="caret-up"
              width="1em"
              height="1em"
              fill="currentColor"
              aria-hidden="true"
            >
              <path d="M858.9 689L530.5 308.2c-9.4-10.9-27.5-10.9-37 0L165.1 689c-12.2 14.2-1.2 35 18.5 35h656.8c19.7 0 30.7-20.8 18.5-35z" />
            </svg>
          </span>
          <span
            role="img"
            aria-label="caret-down"
            className={`anticon anticon-caret-down ant-table-column-sorter-down ${
              isAscending === false && "active"
            }`}
          >
            <svg
              viewBox="0 0 1024 1024"
              focusable="false"
              data-icon="caret-down"
              width="1em"
              height="1em"
              fill="currentColor"
              aria-hidden="true"
            >
              <path d="M840.4 300H183.6c-19.7 0-30.7 20.8-18.5 35l328.4 380.8c9.4 10.9 27.5 10.9 37 0L858.9 335c12.2-14.2 1.2-35-18.5-35z" />
            </svg>
          </span>
        </span>
      </span>
    </>
  );
};

export default SortArrows;
